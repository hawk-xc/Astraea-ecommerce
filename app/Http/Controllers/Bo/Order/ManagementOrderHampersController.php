...
class ManagementOrderHampersController extends Controller
{
    ...
    public function __construct(OrderHampersRepository $repository, OrderHampersDetailRepository $detail_repository, ShippingRepository $shippingRepository, ContactUsRepository $contactUsRepository)
    {
        $this->repository = $repository;
        $this->detail_repository = $detail_repository;
        $this->shippingRepository = $shippingRepository;
        $this->contactUsRepository = $contactUsRepository;
        $this->data['title'] = 'Pesanan Hampers';
        $this->data['view_directory'] = "admin.feature.order.hampers";
    }
    /**
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->editColumn(
                    "total_price",
                    function ($inquiry) {
                        return Helper::to_rupiah($inquiry["total_price"]);
                    }
                )
                ->addColumn('action', function ($row) {
                    $row["id"] = Crypt::encryptString($row["id"]);
                    $btn = '<a href="'. route("order_hampers.show", $row["id"]) .'" class ="btn bg-gradient-primary btn-tooltip mx-1">
    ...
    public function edit(string $id)
    {
        ...
        $data['shipping'] =  Helper::to_rupiah($data['shipping']);
        $data['id'] = Crypt::encryptString($id);
        $ref['url'] = route('order_hampers.update', $data['id']);
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            return redirect()->route('order_hampers.show', $id)->with('success', 'Berhasi mengubah Status Pengiriman');
        } catch (Exception $e) {
            if (env('APP_DEBUG')) {
                return $e->getMessage();
            }
