            ...
            <div class="row">
                <div class="col-lg-8 mb-3 mb-lg-0">
                    <div class="row">
                        <div class="col">
                            ...
                            <p>Alamat :<br>
                                {{ $data['district']->province }}, {{ $data['district']->type }}
                                {{ $data['district']->name }}, {{ auth()->guard('customer')->user()->address }}</p>
                            <hr>
                        </div>
                    </div>
                    <div class="row">
                                        ...
                                        <div class="card">
                                            ...
                                            <div class="card-body">
                                                <div class="d-flex">
                                                    ...
                                                    @endphp
                                                    Payment <span
                                                        class="badge {{ $badgeColor }} text-white m-2">{{ $order->status }}</span>

                                                    @if ($order->status === 'PAID')
                                                        Shipping <span
                                                            class="badge {{ $badgeColor2 }} text-white m-2">{{ $order->shipping_status }}</span>
                                                    @endif
                                                </div>
                                                @if ($order->jenis === 'Product')
                                                    <a href="{{ route('historyp.customer', $order->id) }}"
                                                        class="btn btn-primary mt-2 px-3">Detail</a>
                                                @else
                                                    <a href="{{ route('historyh.customer', $order->id) }}"
                                                        class="btn btn-primary mt-2 px-3">Detail</a>
                                                @endif
                                            </div>
                                        </div>
                ...
                <div class="col-lg-4 mb-3 mb-lg-0">
                    ...
                    <form action="{{ route('logout.customer') }}" method="post">
                        @csrf
                        <a href="{{ route('customer.profiile.edit') }}">
                            <p>Edit Profile</p>
                        </a>
                        <hr>
                        <p><button type="button" data-bs-toggle="modal" data-bs-target="#feedback"
                                class="dashboard">Feedback</button></p>
                        <hr>
                                ...
                                <div class="modal-body">
                                    <div class="image-modal-top">
                                        <div class="container my-3">
                                            <form
                                                action="{{ isset($data['testimoni']) ? route('fo.testimoni.update', $data['s']) : route('fo.testimoni.store') }}"
