			...
			<div class="row">
                <div class="col-md-12">
                    <div class="category-product-filters">
                        <ul>
                            <a href="{{ route('shop-hampers.index') }}">
                            	<li class="{{isset($data['idCategory']) ? '' : 'active'}}">All</li>
                            </a>
                            @foreach($data['categories'] as $category)
                            <a href="{{ route('shop-hampers.category', $category['id']) }}">
	                            <li class="{{(isset($data['idCategory']) && ($data['idCategory'] == $category['id'] )) ? 'active' : ''}}">{{ $category['name'] }}</li>
	                        </a>
                            @endforeach
                        </ul>
                    </div>
				...
				@foreach($data['products'] as $product)
					<div class="col-lg-4 col-md-6 text-center {{ $product['subcategory_id']}}">
						<div class="single-product-item">
							<a href="{{ route('shop-hampers.show', $product['slug']) }}">
								<div class="product-image">
									@if(isset($product['images'][0]['name']))
									<img src="{{ asset('storage/' .$product['images'][0]['name']) }}" class="product-img" alt="">
									@else
							...
							<form action="{{ route('fo.cart-hampers.update', $product['slug']) }}" method="post">
								@csrf
								@method('PUT')
